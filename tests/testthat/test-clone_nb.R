test_that("clone_nb input validation", {
  ...
  expect_error(
    clone_nb(data.frame(gene = is, clone = js), integer(), X),
    "must be a matrix or sparseMatrix"
  )
  expect_error(
    clone_nb(data.frame(gene = is, clone = js), Y, integer()),
    "must be a matrix or sparseMatrix"
  )
  expect_error(
    clone_nb(data.frame(gene = is, clone = js), Y, Y),
    "must be a logical matrix or encoded as 0/1"
  )
  expect_error(
    clone_nb(data.frame(gene = is, clone = js), Y, X[1:10, ]),
